...
diverge_counter = 0;
% Global variables
global R_E T g pitch_ctl me_dot dT STG_m_dot dD D R Qk t_step GPSPVT phi lambda Az R_LS b NS error_flag
error_flag = 0;
%% Global constants
R_E = 6378137; %m
b = 6356752.314; %m
...
for i = 1:length(TIME)-1
    ...
    if any(abs(rsd(1:length(MSMT.PRN)))> 2000)
        diverge_counter = diverge_counter + 1;
    end
     
    if any(abs(rsd(1:length(MSMT.PRN)))> 1e6)
        error_flag = 1;
        break;
    end
...
PR = profile('info');
ex_time = PR.FunctionTable(4).TotalTime/PR.FunctionTable(4).NumCalls;
msmtupdt_time = PR.FunctionTable(18).TotalTime/PR.FunctionTable(18).NumCalls...
...
for i = 1:length(EST.X_est)
    ...
    LV_EKF_data.R_ECEF(i,:) = R_ECEF;
    LV_EKF_data.V_ECEF(i,:) = V_ECEF;
end
error = LV_data.X(1:length(EST.X_est),:) - EST.X_est(:,1:7);
LV_EKF_data.ER = LV_R_ECEF(1:length(LV_EKF_data.R_ECEF),:) - LV_EKF_data.R_ECEF;
LV_EKF_data.EV = LV_V_ECEF(1:length(LV_EKF_data.V_ECEF),:) - LV_EKF_data.V_ECEF;
LV_EKF_data.error = error;
LV_EKF_data.STD = EST.STD;
LV_EKF_data.TIME = TIME;
LV_EKF_data.ExTime = ex_time;
LV_EKF_data.MsmtUpdtT = msmtupdt_time;
save('LV_EKF_data.mat','LV_EKF_data')
