...
for i = 1:length(TIME)- 1
    ...
    GPSPVT = PVT_GPS(i);
    [state_C, rsd] = ESPUKF(STATE(i),MSMT,LV_model);
    if error_flag ==1
        break;
    end
...
for i = 1:length(EST.X_est)
    ...
    LV_ESPUKF_data.R_ECEF(i,:) = R_ECEF;
    LV_ESPUKF_data.V_ECEF(i,:) = V_ECEF;
end
error = LV_data.X(1:length(EST.X_est),:) - EST.X_est(:,1:7);
LV_ESPUKF_data.ER = LV_R_ECEF(1:length(LV_ESPUKF_data.R_ECEF),:) - LV_ESPUKF_data.R_ECEF;
LV_ESPUKF_data.EV = LV_V_ECEF(1:length(LV_ESPUKF_data.V_ECEF),:) - LV_ESPUKF_data.V_ECEF;
LV_ESPUKF_data.error = error;
LV_ESPUKF_data.TIME = TIME;
LV_ESPUKF_data.STD = EST.STD;
LV_ESPUKF_data.ExTime = ex_time;
LV_ESPUKF_data.MsmtUpdtT = msmtupdt_time;
save('LV_ESPUKF_data.mat','LV_ESPUKF_data')
